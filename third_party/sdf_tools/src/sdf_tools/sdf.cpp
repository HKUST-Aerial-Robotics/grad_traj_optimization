...
std::vector<uint8_t>
SignedDistanceField::GetInternalBinaryRepresentation(const std::vector<float>& field_data)
{
  std::vector<uint8_t> raw_binary_data(field_data.size() * 4);
  for(size_t field_index= 0, binary_index= 0; field_index < field_data.size();
      field_index++, binary_index+= 4)
  {
...
std::vector<float>
SignedDistanceField::UnpackFieldFromBinaryRepresentation(const std::vector<uint8_t>& binary)
{
  if((binary.size() % 4) != 0)
  {
  ...
  for(size_t field_index= 0, binary_index= 0; field_index < field_data.size();
      field_index++, binary_index+= 4)
  {
    std::vector<uint8_t> binary_block{ binary[binary_index], binary[binary_index + 1],
                                       binary[binary_index + 2], binary[binary_index + 3] };
...
void SignedDistanceField::FollowGradientsToLocalMaximaUnsafe(
    VoxelGrid::VoxelGrid<Eigen::Vector3d>& watershed_map, const int64_t x_index,
    const int64_t y_index, const int64_t z_index) const
{
  ...
  else
  {
    ...
    else
    {
      ...
      while(true)
      {
        ...
        if(new_stored.x() != -INFINITY && new_stored.y() != -INFINITY &&
           new_stored.z() != -INFINITY)
        {
...
SignedDistanceField::SignedDistanceField(std::string frame, double resolution, double x_size,
                                         double y_size, double z_size, float OOB_value)
...
SignedDistanceField::SignedDistanceField(Eigen::Isometry3d origin_transform, std::string frame,
                                         double resolution, double x_size, double y_size,
                                         double z_size, float OOB_value)
  : initialized_(true), locked_(false)
{
  frame_= frame;
  VoxelGrid::VoxelGrid<float> new_field(origin_transform, resolution, x_size, y_size, z_size,
                                        OOB_value);
  distance_field_= new_field;
}

SignedDistanceField::SignedDistanceField() : initialized_(false), locked_(false)
{
}
...
float SignedDistanceField::Get(const int64_t x_index, const int64_t y_index,
                               const int64_t z_index) const
{
  return distance_field_.GetImmutable(x_index, y_index, z_index).first;
}

std::pair<float, bool> SignedDistanceField::GetSafe(const double x, const double y,
...
std::pair<float, bool> SignedDistanceField::GetSafe(const int64_t x_index, const int64_t y_index,
                                                    const int64_t z_index) const
{
...
/*
 * Setter functions MUST be used carefully - If you arbitrarily change SDF values, it is not a
 * proper SDF any more!
 *
 * Use of these functions can be prevented by calling SignedDistanceField::Lock() on the SDF, at
 * which point these functions will fail with a warning printed to std_err.
 */
...
bool SignedDistanceField::Set(const int64_t x_index, const int64_t y_index, const int64_t z_index,
                              const float value)
{
  if(!locked_)
  {
...
bool SignedDistanceField::CheckInBounds(const int64_t x_index, const int64_t y_index,
                                        const int64_t z_index) const
{
...
SignedDistanceField::GetPrimaryComponentsVector(const Eigen::Vector3d& raw_vector) const
{
  if(std::abs(raw_vector.x()) > std::abs(raw_vector.y()) &&
     std::abs(raw_vector.x()) > std::abs(raw_vector.z()))
  {
    if(raw_vector.x() >= 0.0)
    {
      return std::make_pair(Eigen::Vector3d(GetResolution() * 0.5, 0.0, 0.0),
                            GetResolution() * 0.5);
    }
    else
    {
  ...
  else if(std::abs(raw_vector.y()) > std::abs(raw_vector.x()) &&
          std::abs(raw_vector.y()) > std::abs(raw_vector.z()))
  {
    if(raw_vector.y() >= 0.0)
    {
      return std::make_pair(Eigen::Vector3d(0.0, GetResolution() * 0.5, 0.0),
                            GetResolution() * 0.5);
    }
    else
    {
  ...
  else if(std::abs(raw_vector.z()) > std::abs(raw_vector.x()) &&
          std::abs(raw_vector.z()) > std::abs(raw_vector.y()))
  {
    if(raw_vector.z() >= 0.0)
    {
      return std::make_pair(Eigen::Vector3d(0.0, 0.0, GetResolution() * 0.5),
                            GetResolution() * 0.5);
    }
    else
    {
  ...
  else if(std::abs(raw_vector.x()) == std::abs(raw_vector.y()))
  {
    ...
    return std::make_pair((temp_vector / (temp_vector.norm())) *
                              std::sqrt((GetResolution() * GetResolution() * 0.25) * 2.0),
                          std::sqrt((GetResolution() * GetResolution() * 0.25) * 2.0));
  }
  ...
  {
    return std::make_pair((raw_vector / (raw_vector.norm())) *
                              std::sqrt((GetResolution() * GetResolution() * 0.25) * 3.0),
                          std::sqrt((GetResolution() * GetResolution() * 0.25) * 3.0));
  }
}

double SignedDistanceField::ComputeAxisMatch(const double axis_value,
                                             const double check_value) const
{
...
Eigen::Vector3d SignedDistanceField::GetBestMatchSurfaceVector(
    const Eigen::Vector3d& possible_surfaces_vector,
    const Eigen::Vector3d& center_to_location_vector) const
{
  const Eigen::Vector3d location_rejected_on_possible=
      EigenHelpers::VectorRejection(possible_surfaces_vector, center_to_location_vector);
  // Find the axis with the best-match components
  const double x_axis_match=
      ComputeAxisMatch(possible_surfaces_vector.x(), location_rejected_on_possible.x());
  const double y_axis_match=
      ComputeAxisMatch(possible_surfaces_vector.y(), location_rejected_on_possible.y());
  const double z_axis_match=
      ComputeAxisMatch(possible_surfaces_vector.z(), location_rejected_on_possible.z());
  ...
  {
    std::cerr << "Possible surfaces vector " << possible_surfaces_vector
              << " simple match failed: " << x_axis_match << ", " << y_axis_match << ", "
              << z_axis_match << " (x, y, z)" << std::endl;
    return possible_surfaces_vector;
  }
}

/**
 * @brief GetPrimaryEntrySurfaceVector Estimates the real distance of the provided point, comparing
 * it with the cell center location and gradient vector
 ...
 */
std::pair<Eigen::Vector3d, double> SignedDistanceField::GetPrimaryEntrySurfaceVector(
    const Eigen::Vector3d& boundary_direction_vector,
    const Eigen::Vector3d& center_to_location_vector) const
{
  if(boundary_direction_vector.squaredNorm() > std::numeric_limits<double>::epsilon())
  {
    const std::pair<Eigen::Vector3d, double> primary_components_vector_query=
        GetPrimaryComponentsVector(boundary_direction_vector);
    ...
    else
    {
      ...
      return std::make_pair(GetBestMatchSurfaceVector(primary_components_vector_query.first,
                                                      center_to_location_vector),
                            GetResolution() * 0.5);
    }
  }
  else
...
double SignedDistanceField::EstimateDistanceInternal(const double x, const double y, const double z,
                                                     const int64_t x_idx, const int64_t y_idx,
                                                     const int64_t z_idx) const
{
  const std::vector<double> cell_center= GridIndexToLocation(x_idx, y_idx, z_idx);
  const Eigen::Vector3d cell_center_to_location_vector(x - cell_center[0], y - cell_center[1],
                                                       z - cell_center[2]);
  const double nominal_sdf_distance=
      (double)distance_field_.GetImmutable(x_idx, y_idx, z_idx).first;
  ...
  const std::pair<Eigen::Vector3d, double> entry_surface_information=
      GetPrimaryEntrySurfaceVector(direction_to_boundary, cell_center_to_location_vector);
  ...
  const double center_adjusted_nominal_distance=
      (nominal_sdf_distance >= 0.0) ? nominal_sdf_distance - (GetResolution() * 0.5) :
                                      nominal_sdf_distance + (GetResolution() * 0.5);
  const double minimum_adjusted_distance=
      arc_helpers::SpreadValue(center_adjusted_nominal_distance, -minimum_distance_magnitude, 0.0,
                               minimum_distance_magnitude);

  // Account for target location being not at the exact center of the voxel
  const double raw_distance_adjustment=
      EigenHelpers::VectorProjection(entry_surface_vector, cell_center_to_location_vector).norm();
  const double real_distance_adjustment=
      (minimum_adjusted_distance >= 0.0) ? -raw_distance_adjustment : raw_distance_adjustment;
  ...
  {
    std::cerr << "Center adjusted nominal distance " << minimum_adjusted_distance
              << " final adjusted_distance " << final_adjusted_distance << std::endl;
    assert(false && "Mismatched minimum and final adjusted distance signs");
  }
}

std::pair<double, bool> SignedDistanceField::EstimateDistance(const double x, const double y,
                                                              const double z) const
{
...
SignedDistanceField::EstimateDistance3d(const Eigen::Vector3d& location) const
{
  const std::vector<int64_t> indices= LocationToGridIndex3d(location);
  if(indices.size() == 3)
  {
    return std::make_pair(EstimateDistanceInternal(location.x(), location.y(), location.z(),
...
std::pair<double, bool>
SignedDistanceField::EstimateDistance4d(const Eigen::Vector4d& location) const
{
  const std::vector<int64_t> indices= LocationToGridIndex4d(location);
  if(indices.size() == 3)
  {
    return std::make_pair(EstimateDistanceInternal(location(0), location(1), location(2),
                                                   indices[0], indices[1], indices[2]),
                          true);
  }
  else
...
std::vector<double> SignedDistanceField::GetGradient(const double x, const double y, const double z,
                                                     const bool enable_edge_gradients) const
{
  return GetGradient4d(Eigen::Vector4d(x, y, z, 1.0), enable_edge_gradients);
}

std::vector<double> SignedDistanceField::GetGradient3d(const Eigen::Vector3d& location,
                                                       const bool enable_edge_gradients) const
{
  const std::vector<int64_t> indices= LocationToGridIndex3d(location);
  if(indices.size() == 3)
...
std::vector<double> SignedDistanceField::GetGradient4d(const Eigen::Vector4d& location,
                                                       const bool enable_edge_gradients) const
{
  const std::vector<int64_t> indices= LocationToGridIndex4d(location);
  if(indices.size() == 3)
...
std::vector<double> SignedDistanceField::GetGradient(const VoxelGrid::GRID_INDEX& index,
                                                     const bool enable_edge_gradients) const
{
  return GetGradient(index.x, index.y, index.z, enable_edge_gradients);
}

std::vector<double> SignedDistanceField::GetGradient(const int64_t x_index, const int64_t y_index,
                                                     ...
                                                     const bool enable_edge_gradients) const
{
     ...
     (y_index < GetNumYCells()) && (z_index < GetNumZCells()))
  {
    ...
    if((x_index > 0) && (y_index > 0) && (z_index > 0) && (x_index < (GetNumXCells() - 1)) &&
       (y_index < (GetNumYCells() - 1)) && (z_index < (GetNumZCells() - 1)))
    {
      // double inv_twice_resolution = 1.0 / (2.0 * GetResolution());
      // double gx = (Get(x_index + 1, y_index, z_index) - Get(x_index - 1, y_index, z_index)) *
      // inv_twice_resolution; double gy = (Get(x_index, y_index + 1, z_index) - Get(x_index,
      // y_index - 1, z_index)) * inv_twice_resolution; double gz = (Get(x_index, y_index, z_index +
      // 1) - Get(x_index, y_index, z_index - 1)) * inv_twice_resolution;

      // use scharr instead
      double resolution= GetResolution();
      double gx=
          (10 * (Get(x_index + 1, y_index, z_index) - Get(x_index - 1, y_index, z_index)) +
           3 * (Get(x_index + 1, y_index + 1, z_index) - Get(x_index - 1, y_index + 1, z_index)) +
           3 * (Get(x_index + 1, y_index - 1, z_index) - Get(x_index - 1, y_index - 1, z_index))) /
          (30 * resolution);
      double gy=
          (10 * (Get(x_index, y_index + 1, z_index) - Get(x_index, y_index - 1, z_index)) +
           3 * (Get(x_index + 1, y_index + 1, z_index) - Get(x_index + 1, y_index - 1, z_index)) +
           3 * (Get(x_index - 1, y_index + 1, z_index) - Get(x_index - 1, y_index - 1, z_index))) /
          (30 * resolution);
      double gz=
          (10 * (Get(x_index, y_index, z_index + 1) - Get(x_index, y_index, z_index - 1)) +
           3 * (Get(x_index, y_index + 1, z_index + 1) - Get(x_index, y_index + 1, z_index - 1)) +
           3 * (Get(x_index, y_index - 1, z_index + 1) - Get(x_index, y_index - 1, z_index - 1))) /
          (30 * resolution);
...
Eigen::Vector3d SignedDistanceField::ProjectOutOfCollision(const double x, const double y,
                                                           const double z,
                                                           const double stepsize_multiplier) const
{
  const Eigen::Vector4d result=
      ProjectOutOfCollision4d(Eigen::Vector4d(x, y, z, 1.0), stepsize_multiplier);
  return result.head<3>();
}

Eigen::Vector3d SignedDistanceField::ProjectOutOfCollisionToMinimumDistance(
    const double x, const double y, const double z, const double minimum_distance,
    const double stepsize_multiplier) const
{
  const Eigen::Vector4d result= ProjectOutOfCollisionToMinimumDistance4d(
      Eigen::Vector4d(x, y, z, 1.0), minimum_distance, stepsize_multiplier);
  return result.head<3>();
}

Eigen::Vector3d SignedDistanceField::ProjectOutOfCollision3d(const Eigen::Vector3d& location,
                                                             const double stepsize_multiplier) const
{
  return ProjectOutOfCollision(location.x(), location.y(), location.z(), stepsize_multiplier);
}

Eigen::Vector3d SignedDistanceField::ProjectOutOfCollisionToMinimumDistance3d(
    const Eigen::Vector3d& location, const double minimum_distance,
    const double stepsize_multiplier) const
{
  return ProjectOutOfCollisionToMinimumDistance(location.x(), location.y(), location.z(),
                                                minimum_distance, stepsize_multiplier);
}

Eigen::Vector4d SignedDistanceField::ProjectOutOfCollision4d(const Eigen::Vector4d& location,
                                                             const double stepsize_multiplier) const
{
  return ProjectOutOfCollisionToMinimumDistance4d(location, 0.0, stepsize_multiplier);
}

Eigen::Vector4d SignedDistanceField::ProjectOutOfCollisionToMinimumDistance4d(
    const Eigen::Vector4d& location, const double minimum_distance,
    const double stepsize_multiplier) const
{
  // To avoid potential problems with alignment, we need to pass location by reference, so we make a
  // local copy here that we can change.
...
std::vector<int64_t>
SignedDistanceField::LocationToGridIndex3d(const Eigen::Vector3d& location) const
{
  return distance_field_.LocationToGridIndex3d(location);
}

std::vector<int64_t>
SignedDistanceField::LocationToGridIndex4d(const Eigen::Vector4d& location) const
{
  return distance_field_.LocationToGridIndex4d(location);
}

std::vector<int64_t> SignedDistanceField::LocationToGridIndex(const double x, const double y,
                                                              const double z) const
{
...
std::vector<double>
SignedDistanceField::GridIndexToLocation(const VoxelGrid::GRID_INDEX& index) const
{
  return distance_field_.GridIndexToLocation(index);
}

std::vector<double> SignedDistanceField::GridIndexToLocation(const int64_t x_index,
...
bool SignedDistanceField::LoadFromMessageRepresentation(const SDF& message)
{
  ...
  Eigen::Translation3d origin_translation(message.origin_transform.translation.x,
                                          message.origin_transform.translation.y,
                                          message.origin_transform.translation.z);
  Eigen::Quaterniond origin_rotation(
      message.origin_transform.rotation.w, message.origin_transform.rotation.x,
      message.origin_transform.rotation.y, message.origin_transform.rotation.z);
  Eigen::Isometry3d origin_transform= origin_translation * origin_rotation;
  VoxelGrid::VoxelGrid<float> new_field(origin_transform, message.sdf_cell_size,
                                        message.dimensions.x, message.dimensions.y,
                                        message.dimensions.z, message.OOB_value);
    ...
    for(int64_t y_index= 0; y_index < distance_field_.GetNumYCells(); y_index++)
    {
      for(int64_t z_index= 0; z_index < distance_field_.GetNumZCells(); z_index++)
      {
        ...
        std::vector<double> location=
            distance_field_.GridIndexToLocation(x_index, y_index, z_index);
...
SignedDistanceField::ExportForDisplayCollisionOnly(const float alpha) const
{
  // Assemble a visualization_markers::Marker representation of the SDF to display in RViz
  visualization_msgs::Marker display_rep;
  // Populate the header
...
{
  VoxelGrid::VoxelGrid<Eigen::Vector3d> watershed_map(
      GetOriginTransform(), GetResolution(), GetXSize(), GetYSize(), GetZSize(),
      Eigen::Vector3d(-INFINITY, -INFINITY, -INFINITY));
  for(int64_t x_idx= 0; x_idx < watershed_map.GetNumXCells(); x_idx++)
  {
    for(int64_t y_idx= 0; y_idx < watershed_map.GetNumYCells(); y_idx++)
    {
      ...
      {
        // We use an "unsafe" function here because we know all the indices we provide it will be
        // safe
...
{
  // A gradient is at a local maxima if the absolute value of all components (x,y,z) are less than
  // 1/2 SDF resolution
  double half_resolution= GetResolution() * 0.5;
  if(std::fabs(gradient.x()) <= half_resolution && std::fabs(gradient.y()) <= half_resolution &&
     std::fabs(gradient.z()) <= half_resolution)
  {
    return true;
  }
  else
...
VoxelGrid::GRID_INDEX SignedDistanceField::GetNextFromGradient(
    const VoxelGrid::GRID_INDEX& index, const Eigen::Vector3d& gradient) const
{
